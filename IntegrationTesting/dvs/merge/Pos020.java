...
/**
 * Create two early binding portgroups (each having 2 ports) having the same
 * names with two VMvnics connected to each different portgroups having
 * different portNameFormat and no conflicting ports. Merge the switches
 */
public class Pos020 extends TestBase
{
   ...
   public void setTestDescription()
   {
      setTestDescription("Create two early binding portgroups (each having "
               + "2 ports) having the same names with two VMvnics connected to "
      ...
      throws Exception
   {
      boolean status = true;
          ...
          */
         log.info("Restoring the network configuration of the host");
         ...
         if (this.iManagedEntity.isExists(this.srcDvsMor)) {
            ...
            if (this.iManagedEntity.destroy(this.srcDvsMor)) {
               log.info("Destroyed the source DVS ");
            } else {
               status &= false;
            }
         }
