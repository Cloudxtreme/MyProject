...
/**
 * Create two late binding portgroups having the same names with two VMvnics
 * connected to each different portgroups having different portNameFormat and no
 * conflicting ports. Merge the switches
 */
public class Pos022 extends TestBase
{
   ...
   private boolean checkGuest = DVSTestConstants.CHECK_GUEST;
   ...
   {
      setTestDescription("Create two late binding portgroups having the same "
               + "names with two VMvnics connected to each different portgroups "
               + "having different portNameFormat and no conflicting ports. "
               + "Merge the switches");
   }
    ...
    * @param connectAnchor ConnectAnchor object
    */
   @Test(description = "Create two late binding portgroups having the same "
               + "names with two VMvnics connected to each different portgroups "
               + "having different portNameFormat and no conflicting ports. "
               + "Merge the switches")
   public void test()
      ...
      boolean status = false;
         ...
         if (this.iDVSwitch.merge(destDvsMor, srcDvsMor)) {
            ...
            if (this.iNetworkSystem.refresh(this.secondNetworkMor)
               ...
               log.info("Successfully refreshed the host network");
               ...
               log.error("Can not refresh the network config of the host");
            }
         } else {
                        ...
                        if (vmConfigSpec != null && vmConfigSpec.length == 2
                           ...
                           if (setUpDone) {
                              log.info("Successfully reconfigured"
                                       + " the VM to use the DV Portgroup");
                              setUpDone = this.ivm.powerOnVM(this.vmMor, null,
                                       checkGuest);
                              if (checkGuest) {
                                 setUpDone = DVSUtil.checkNetworkConnectivity(
                                          this.iHostSystem.getIPAddress(srcDvsHostMor),
                                          this.ivm.getIPAddress(this.vmMor),
                                          true);
                                 if (setUpDone) {
                                    log.info("Successfully verified the network"
                                             + " connectivity of the VM "
                                             + this.ivm.getVMName(this.vmMor));
                                 } else {
                                    log.error("Can not check the network "
                                             + "connectivity of the VM "
                                             + this.ivm.getVMName(this.vmMor));
                                 }
                              }
