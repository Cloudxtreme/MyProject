 ...
 * different policies.
 */
public class Pos026 extends TestBase
{
   ...
   public void setTestDescription()
   {
      setTestDescription("Create two early binding portgroups having different"
               + " names with two VMKnics connected to each one of the"
               + " portgroups having different policies");
   }
         ...
         if (hosts != null && hosts.size() >= 2) {
            ...
            if (this.destnDvsHostMor != null && this.srcDvsHostMor != null) {
               ...
               if (this.rootFolderMor != null) {
                  ...
                  this.srcfolder = this.iFolder.createFolder(networkFolder,
      ...
      throws Exception
   {
      ...
      DVPortSetting defaultPortConfig = new DVPortSetting();
      defaultPortConfig.setBlocked(DVSUtil.getBoolPolicy(false, false));
      DVSTrafficShapingPolicy shaping = null;
      shaping = DVSUtil.getTrafficShapingPolicy(false, true, 100000L, 100000L,
               100000L);
      defaultPortConfig.setOutShapingPolicy(shaping);
      spec.setDefaultPortConfig(defaultPortConfig);
            ...
            if (setUpDone) {
               ...
               if (vdConfigSpec != null) {
                  ...
                  if (numCards > 0) {
                     ...
                     spec.setConfigVersion("");
                     DVPortSetting defaultPortConfig = new DVPortSetting();
                     defaultPortConfig.setBlocked(DVSUtil.getBoolPolicy(false,
                              false));
                     DVSTrafficShapingPolicy shaping = DVSUtil.getTrafficShapingPolicy(
                              false, true, 102400L, 102400L, 102400L);
                     defaultPortConfig.setOutShapingPolicy(shaping);
                     spec.setDefaultPortConfig(defaultPortConfig);
                     ...
                     if (pg != null && pg.size() > 0 && pg.get(0) != null) {
                        portgroupKey = this.idvpg.getConfigInfo(pg.get(0)).getKey();
