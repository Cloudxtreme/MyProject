...
/**
 * Create two late binding portgroups having the different names with two
 * VMvnics connected to each different portgroups having different
            ...
            if (this.destnDvsHostMor != null && this.srcDvsHostMor != null) {
               ...
               if (this.destDvsMor != null) {
                  ...
                  if (this.srcDvsMor != null) {
                     this.srcDVSName = this.iDVSwitch.getConfig(this.srcDvsMor).getName();
      ...
      boolean status = false;
         ...
         if (this.iDVSwitch.merge(destDvsMor, srcDvsMor)) {
            ...
            if (this.iNetworkSystem.refresh(this.secondNetworkMor)) {
               log.info("Succesfully refershed the network system of the host");
                        ...
                        if (vmConfigSpec != null && vmConfigSpec.length == 2
                           ...
                           if (setUpDone) {
                              ...
                              setUpDone = this.ivm.powerOnVM(this.vmMor, null,
                                       checkGuest);
                              if (setUpDone) {
                                 log.info("Succesfully powered on the VM "
                                          + this.vmName);
                                 if (checkGuest) {
                                    setUpDone = DVSUtil.checkNetworkConnectivity(
                                             this.iHostSystem.getIPAddress(this.srcDvsHostMor),
                                             this.ivm.getIPAddress(this.vmMor),
                                             true);
                                    if (setUpDone) {
                                       log.info("Successfully verified the "
