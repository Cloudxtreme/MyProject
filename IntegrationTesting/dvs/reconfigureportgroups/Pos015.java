 ...
 * switch with scope set to cluster compute resource mor
 */
public class Pos015 extends TestBase
{
      ...
      throws Exception
   {
      ...
      try {
         ...
         if (this.dcMor != null) {
            allClusters = this.iComputeResource.getAllClusters();
            if (allClusters != null && allClusters.size() >= 1) {
               log.info("Successfully found compute resource mor "
                        + "objects ");
               this.clusterComputeResourceMor = (ManagedObjectReference) allClusters.get(0);
            }
            if (this.clusterComputeResourceMor == null) {
               log.info("Can not find a cluster in the setup");
               log.info("Creating a new cluster " + this.getTestId()
                        + "-Cluster");
               this.clusterComputeResourceMor = this.iFolder.createCluster(
                        this.iFolder.getHostFolder(this.dcMor),
                        this.getTestId() + "-Cluster", new ClusterConfigSpec());
            ...
            if (this.clusterComputeResourceMor != null) {
               ...
               if (clusterHosts == null || clusterHosts.size() == 0) {
                  ...
                  this.isHostMoved = this.iComputeResource.moveHostInto(
                           this.clusterComputeResourceMor, this.hostMor, null);
               } else {
                  this.hostMor = clusterHosts.get(0);
               }
               ...
               log.error("Failed to find a cluster compute "
                        + "resource Mor");
            }
         } else {
            log.error("Can not find a valid data center in the setup");
         }
      } catch (Exception e) {
            ...
            if (portgroupKey != null) {
               ...
               if (!isEesx) {
                  portConnection = this.iDVSwitch.getPortConnection(dvsMor,
                           null, false, null, new String[] { portgroupKey });
                  /*
      ...
      throws Exception
   {
      ...
      try {
         ...
         if (isClusterCreated) {
            status &= this.iManagedEntity.destroy(this.clusterComputeResourceMor);
         }
         /*
