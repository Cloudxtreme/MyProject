 ...
 * with a valid name that contains all alphabets, numeric characters, and
 * special characters
 */
public class Pos053 extends TestBase
{
   ...
   public void setTestDescription()
   {
      setTestDescription("Reconfigure an existing portgroup on an existing "
               + "distributed virtual switch with a valid name that"
               + " contains all alphabets, numeric characters, and "
               + "special characters");
   }
      ...
      throws Exception
   {
      ...
      try {
         if (dvPortgroupMorList != null && dvPortgroupMorList.size() > 0) {
            ...
            this.dvPortgroupConfigSpec.setName(DVSTestConstants.NAME_ALPHA_NUMERIC_SPECIAL_CHARS);
