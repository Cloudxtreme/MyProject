...
/**
 * Reconfigure a portgroup to an existing distributed virtual switch with
 * multiple vlanid ranges in the default port config.
 */
public class Pos059 extends TestBase
{
   ...
   public void setTestDescription()
   {
      setTestDescription("Reconfigure a portgroup to an existing distributed "
               + "virtual switch with multiple vlanid ranges in the "
               + "default port config");
   }
      ...
      throws Exception
   {
      ...
      NumericRange[] numericRanges = new NumericRange[2];
      ...
      try {
         ...
         numericRange.setStart(30);
         numericRange.setEnd(45);
         numericRanges[0] = numericRange;
         numericRange = new NumericRange();
         numericRange.setStart(70);
         numericRange.setEnd(80);
         numericRanges[1] = numericRange;
         vlanspec.getVlanId().clear();
         vlanspec.getVlanId().addAll(com.vmware.vcqa.util.TestUtil.arrayToVector(numericRanges));
         this.dvPortSetting = (VMwareDVSPortSetting) this.iDVPortgroup.getConfigInfo(
                  dvPortgroupMorList.get(0)).getDefaultPortConfig();
         this.dvPortSetting.setVlan(vlanspec);
         this.dvPortgroupConfigSpec.setDefaultPortConfig(this.dvPortSetting);
