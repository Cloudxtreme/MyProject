 ...
 * <br>
 * TEST:<br>
 * 6. Generate different combinations of VSPAN sessions using DVPorts with no
 ...
 */
public class Pos009 extends VspanTestBase implements IDataDrivenTest
{
   VMwareVspanSession[] existingSessions;

   @BeforeClass(dependsOnMethods = { "preSetUp" })
   public void getHost()
      throws Exception
   {
      initialize();
   ...
   @Test(description = "EDIT a VSPAN session to DVS by providing different "
            + "options for source port Tx and Rx with no destination options")
   @Override
   public void test()
      throws Exception
   {
      // sessions will be created with standalone port as destination.
      final List<String> ports = vmwareDvs.addStandaloneDVPorts(dvsMor, 1);
      final VMwareVspanPort vspanPort = VspanHelper.buildVspanPort(ports.get(0) , null,
               null);
      final VMwareDVSVspanConfigSpec[] specs = buildVspanCfgs(vspanPort, dvsMor);
      for (int i = 0; i < specs.length; i++) {
         ...
         edit.setDestinationPort(null);// set null here
