...
/**
 * Move n DVPorts (not bound to any VM) where some standalone and some are from
 * early binding DVPortGroup to a late binding DVPortGroup. Move DVPorts
 * (standalone & early) -> late bind port group.
 */
public class Pos021 extends MovePortBase
{
   ...
   {
      setTestDescription("Move n DVPorts (not bound to any VM) where some "
               + "standalone and some are from early binding DVPortGroup "
    ...
    * added DVPortgroup as ports to move. 5. Create late binding DVPortGroup and
    * use its key as destination.
    * 
            ...
            if (portKeys.size() >= numPorts) {
               ...
               if ((portgroupKeys != null) && !portgroupKeys.isEmpty()) {
                  ...
                  if ((portgroupKeys != null) && !portgroupKeys.isEmpty()) {
                     ...
                     log.error("faile to add late binding DVPortgroup.");
                  }
               } else {
    ...
    */
   @Override
   @Test(description = "Move n DVPorts (not bound to any VM) where some "
               + "standalone and some are from early binding DVPortGroup "
               + "to a late binding DVPortGroup.")
   public void test()
      throws Exception
