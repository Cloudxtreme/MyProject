...
/**
 * Move a conflict standalone DVPort to DVPORTGROUP.
 */
public class Neg007 extends MovePortBase
{
   ...
   {
      setTestDescription("Move a conflict standalone DVPort to DVPORTGROUP.");
   }
    ...
    */
   @Override
   @Test(description = "Move a conflict standalone DVPort to DVPORTGROUP.")
   public void test()
      throws Exception
   {
      boolean status = false;
      MethodFault expectedFault = new NotSupported();
      try {
         portgroupKey = iDVSwitch.addPortGroup(dvsMor,
                  DVPORTGROUP_TYPE_EARLY_BINDING, 1, getTestId() + "-pg1");
         movePort(dvsMor, portKeys, portgroupKey);// try move to DVS.
      ...
      throws Exception
   {
      ...
      try {
         ...
         if ((this.secondHostMor != null)
                  ...
                  && (this.conn != null)) {
            if (this.ihs.isEesxHost(this.secondHostMor)) {
               vpxaCommand = TestConstants.EESX_SSHCOMMAND_STARTVPXA;
            } else {
               vpxaCommand = TestConstants.SSHCOMMAND_STARTVPXA;
            }
            if (SSHUtil.executeRemoteSSHCommand(this.conn, vpxaCommand)) {
               while (!this.ihs.isHostConnected(this.secondHostMor)
                        && (timeout > 0)) {
                  Thread.sleep(1500);
                  timeout--;
               }
            ...
            if (hostLoginSession != null) {
               ...
               haVirtualMachine = new VirtualMachine(hostConnectAnchor);
                     if (this.ihs.disconnectHost(this.secondHostMor)) {
                        ...
                        haHostMor = haHostSystem.getConnectedHost(false);
                        if (haHostMor != null) {
                           ...
                           if (ihostDVSManager.hostDVSUpdatePorts(
                                    hostDVSManager, this.dvsUuid,
                                    new HostDVSPortData[] { portData })) {
                  ...
                  if (this.vm2DeltaCfgSpec != null) {
                     if (this.ihs.reconnectHost(secondHostMor, hostConnectSpec, null)) {
                           portCriteria = this.iDVSwitch.getPortCriteria(null,
                                    null, null, null, null, false);
                           if (this.iDVSwitch.refreshPortState(this.dvsMor,
                                    null)) {
                              ports = this.iDVSwitch.fetchPorts(this.dvsMor,
                                       portCriteria);
                              if ((ports != null) && (ports.size() >= 2)) {
                                 for (DistributedVirtualPort port : ports) {
                                    if ((port != null)
                                             && port.isConflict()
                                             && portKey.equals(port.getConflictPortKey())) {
                  ...
                  log.error("Can not open a ssh connection to the host "
                           + this.secondHostName);
               }
            } else {
