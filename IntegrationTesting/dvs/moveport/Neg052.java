 ...
 * with livePortMovingAllowed = false, to a late binding DVPortgroup with
 ...
 * Create late binding DVPortgroup with livePortMovingAllowed = false and
 * connect the VM to a DVPort in it and use it as port to be moved.</br> 4.
 * Verify power ops the VM.</br> 5. Create late binding DVPortGroup with
 * livePortMovingAllowed = true and use it as destination.</br> Test:</br> 6.
 * Move the DVPort to destination, Should fail.</br> Cleanup:</br> 7. Delete the
 * DVS and logout.</br>
 */
public class Neg052 extends MovePortBase
{
      ...
      throws Exception
   {
      ...
      try {
         status = ivm.setVMState(vm1Mor, POWERED_ON, false);
