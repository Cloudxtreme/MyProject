...
/**
 * The following setup will be used for BackupRestore tests
 * However, the setup will be modified as necessary later
 * 1. Create 2 DVS (DVS1 and DVS2)
 * 2. create ephemeral port groups DVPG1 on DVS1 and DVPG2 on DVS2
...
public class Pos001 extends TestBase
{
   ...
   {
      super.setTestDescription("export vDS config with no port groups\n"
               + " 1. Create 2 DVS (DVS1 and DVS2)\n"
    ...
    */
   @Override
   @Test(description = "export vDS config with no port groups\n"
               + " 1. Create 2 DVS (DVS1 and DVS2)\n"
               + " 2. create ephemeral portgroups DVPG1 on DVS1 and DVPG2 on DVS2\n"
               + " 3. create a VM with 2 vNics\n"
               + " 4. Define DVPG1 as the backing for vNic1 and DVPG2 as the backing for vNic2\n"
      ...
      try {
         ...
         if (freePnics != null && freePnics.length >= 2) {
            ...
            for (ManagedObjectReference mor : dvsMorList) {
               ...
               log.debug("Current Selection Count index is " + count);
            ...
            Vector<ManagedObjectReference> dList = new Vector<ManagedObjectReference>(1);
            dList.add(dvsMor);
            dList.add(srcDVSMor);
            deleteDVSConfiguration(dList);
            dvsMorList.remove(dvsMor);
            dvsMorList.remove(srcDVSMor);
            iDVSMgr.importEntity(dvsManagerMor, vDSConfig, "createEntityWithOriginalIdentifier");
            /* Get the list of DVSes and cleanup it up */
            dvsMorList = (Vector<ManagedObjectReference>)
                       iFolder.getAllDistributedVirtualSwitch(iFolder.getNetworkFolder(dcMor));
   ...
   public boolean deleteDVSConfiguration(Vector<ManagedObjectReference> dvsMORList)
   throws Exception
   {
      boolean status = true;
      try {
            ...
            if (allVMs != null && !allVMs.isEmpty()) {
               for (int i=0; i<allVMs.size(); i++) {
                  ...
                  if (ivm.getName(currVmMor).contains(pattern)) {
                     log.info("VM " + ivm.getName(currVmMor) + "will be cleaned up");
                     if (ivm.setVMState(currVmMor, VirtualMachinePowerState.POWERED_OFF, false)) {
                        if (isVMCreated) {
                           log.info("Destroying the created VM" + ivm.getName(currVmMor));
                           status &= ivm.destroy(currVmMor);
                        }
                     } else {
                        log.error("Can not power off the VM" + ivm.getName(currVmMor));
                        status &= false;
                     }
                  }
               }
