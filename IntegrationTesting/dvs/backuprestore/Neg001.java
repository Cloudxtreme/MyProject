...
/*   SelectionSet[] selectionSet = new SelectionSet[2];*/
   ...
   {
      super.setTestDescription("Neg001: Call exportEntity with null selectionSet\n"
               + " 1. Create 2 DVS (DVS1 and DVS2)\n"
   ...
   @Test(description = "Neg001: Call exportEntity with null selectionSet\n"
               + " 1. Create 2 DVS (DVS1 and DVS2)\n"
               + " 2. create ephemeral portgroups DVPG1 on DVS1 and DVPG2 on DVS2\n"
               + " 3. create a VM with 2 vNics\n"
               + " 4. Define DVPG1 as the backing for vNic1 and DVPG2 as the backing for vNic2\n"
         ...
         try {
            ...
            if (freePnics != null && freePnics.length >= 2) {
               ...
               for (ManagedObjectReference mor : dvsMorList) {
                  ...
                  log.info("The DVS UUID is " + dvSwitchUuid);
                  dvsSS.setDvsUuid(dvSwitchUuid);
                  ephemeral = idvpg.getKey(ephepg);
                  dvpgSS.setDvsUuid(dvSwitchUuid);
                  dvpgSS.getPortgroupKey().add(ephemeral);
