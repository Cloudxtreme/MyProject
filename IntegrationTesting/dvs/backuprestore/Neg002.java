...
/**
 * TODO: Revisit the setup
 * We do not need this big set for this test, simplify it
...
public class Neg002 extends TestBase
{
   ...
   {
	   super.setTestDescription("Neg002: Call exportEntity with unsupported hostmember selection set\n"
               + " 1. Create 2 DVS (DVS1 and DVS2)\n"
    ...
    */
   @Override
   @Test(description = "Call exportEntity with unsupported hostmember selection set\n"
               + " 1. Create 2 DVS (DVS1 and DVS2)\n"
               + " 2. create ephemeral portgroups DVPG1 on DVS1 and DVPG2 on DVS2\n"
               + " 3. create a VM with 2 vNics\n"
               + " 4. Define DVPG1 as the backing for vNic1 and DVPG2 as the backing for vNic2\n"
          ...
          String[] freePnics = ins.getPNicIds(hostMor);
            if (freePnics != null && freePnics.length >= 2) {
               ...
               for (ManagedObjectReference mor : dvsMorList) {
                  ...
                  HostMemberSelection hmSS = new HostMemberSelection();
                  hmSS.setDvsUuid(dvSwitchUuid);
                  selectionSet[count++] = hmSS;
