...
public class Pos009 extends TestBase {
	...
	public void setTestDescription() {
		super.setTestDescription("Test import API with applyToEntitySpecified import type of reconfiguring DVS with no port groups\n"
				+ " 1. Create 2 DVS (DVS1 and DVS2)\n"
				+ " 2. create a VM with 2 vNics\n"
				+ " 3. Export DVS1 configuration\n"
				+ " 4. Reconfigure DVS2 configuration\n"
				+ " 5. Delete DVS2 configuration and apply DVS2 configurarion to DVS1 using applyToEntitySpecified import type\n");
	}
	 ...
	 */
	@Override
	@Test(description = "Test import API with applyToEntitySpecified import type of reconfiguring DVS with no port groups\n"
			+ " 1. Create 2 DVS (DVS1 and DVS2)\n"
			+ " 2. create a VM with 2 vNics\n"
			+ " 3. Export DVS1 configuration\n"
			+ " 4. Reconfigure DVS2 configuration\n"
	...
	public void test() throws Exception {
		 ...
		 * DVS2 Now apply the DVS1 configuration to DVS2 configuration to
		 * over-write the DVS2 configuration
		 */
		...
		try {
			...
			if (freePnics != null && freePnics.length >= 2) {
				...
				DVS1Config = iDVSMgr.exportEntity(dvsManagerMor, selectionSet);
				...
				DVS2Config = iDVSMgr.exportEntity(dvsManagerMor,
						dvs2SelectionSet);
				DVS2Config[0].setKey(DVS1Config[0].getKey());

				Vector<ManagedObjectReference> dList = new Vector<ManagedObjectReference>(
						1);
				dList.add(dvsMor);
				deleteDVSConfiguration(dList);
				dvsMorList.remove(dvsMor);
				/*
				 * remove this from mor list, otherwise, cleanup with throw an
				 * exception
				 */
				iDVSMgr.importEntity(dvsManagerMor, DVS2Config,
	...
	public boolean reconfigureDVSBasic(ManagedObjectReference dvsMOR)
		...
		if (dvsMOR != null) {
			...
			contactInfo.setName("Contact "
					+ DVSTestConstants.DVS_RECONFIG_NAME_PREFIX + " Pos007");
			...
			deltaConfigSpec
					.setDescription("modified the config for applytoentity specified import type");
