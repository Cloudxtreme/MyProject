...
public class Pos003 extends TestBase {
	...
	public void setTestDescription() {
		super.setTestDescription("Test export functionality with both vDS and port groups\n"
				+ " 1. Create DVS'es as many as the free pNICS available\n"
				+ " 2. create ephemeral portgroups DVPG1 on all the DVSes\n"
				+ " 3. Export the complete networking configuration\n"
				+ " 4. Delete the network configuration\n"
				+ " 5. Import the saved configuration and verify the original configuration is restored\n");
	}
	 ...
	 */
	@Override
	@Test(description = "Test export functionality with both vDS and port groups\n"
			+ " 1. Create DVS'es as many as the free pNICS available\n"
			+ " 2. Create ephemeral portgroups DVPG1 on all the DVSes\n"
			+ " 3. Export the complete networking configuration\n"
			+ " 4. Delete the network configuration\n"
		...
		try {
			...
			if (freePnics != null && freePnics.length >= 2) {
               ...
               dvpgSS.getPortgroupKey().addAll(com.vmware.vcqa.util.TestUtil.arrayToVector(ephemeral));
				...
				// Get the network configuration and verify the import worked
				// fine
