...
/**
 * Convert a VM that connects to a DVPort in an early binding portgroup into
 * template. The template will contain the early binding portgroup information,
 * and it will not contain any DVPort information to connect to. Clone a VM from
 * the template and power on the VM.
 */
public class Pos013 extends FunctionalTestBase
{
   ...
   {
      setTestDescription("Convert a VM that connects to a DVPort in an early "
               + "binding portgroup into template. The template will "
               + "contain the early binding portgroup information, "
               + "and it will not contain any DVPort information to "
               + "connect to. Clone two VM's from the template and "
               + "power on the two VM's.");
   }
                        ...
                        if (numCards > 0) {
                           ...
                           if (this.pgKey != null) {
                              portCriteria = this.iDVS.getPortCriteria(false,
                                       null, null, new String[] { this.pgKey },
                                       null, true);
                                 ...
                                 if (vmConfigSpec != null
                                    ...
                                    if (setUpDone) {
                                       ...
                                       setUpDone = this.ivm.setVMState(
                                                this.vmMor, POWERED_ON, checkGuest);
                                       if (setUpDone) {
    ...
    * @param connectAnchor ConnectAnchor object
    */
   @Test(description = "Convert a VM that connects to a DVPort in an early "
               + "binding portgroup into template. The template will "
               + "contain the early binding portgroup information, "
               + "and it will not contain any DVPort information to "
               + "connect to. Clone two VM's from the template and "
      ...
      String ipAddress = null;
         ...
         if (testDone) {
            ...
            this.cloneVMMor = this.ivm.cloneVM(this.vmMor,
                     this.ivm.getVMFolder(), this.vmName + "-Clone1", cloneSpec);
               ...
               if (testDone) {
                  ...
                  if (testDone) {
                     ...
                     assertTrue(InternalDVSHelper.verifyPortPersistenceLocation(
                              ...
                              this.dvSwitchUUID),
                              "Verification for ortPersistenceLocation failed");
