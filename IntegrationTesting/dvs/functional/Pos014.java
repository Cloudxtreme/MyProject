...
/**
 * Convert a VM that connects to an early binding DVPortgroup into template. The
 * template will contain the information of the early binding portgroup to
 * connect to. Clone two VM's from the template and power on the two VM's.
 */
public class Pos014 extends FunctionalTestBase
{
   ...
   {
      setTestDescription("Convert a VM that connects to an early binding "
               + "DVPortgroup into template. The tempalte will "
               + "contain the information of the early binding "
               + "portgroup to connect to. Clone two VM's from the "
               + "template and power on the two VM's.");
   }
                  ...
                  if (vdConfigSpec != null) {
                     ...
                     if (numCards > 0) {
                        this.pgKey = this.iDVS.addPortGroup(this.dvsMor,
                                 DVPORTGROUP_TYPE_EARLY_BINDING, 2 * numCards,
                                 this.getTestId() + "-pg1");
                        if (this.pgKey != null) {
                              ...
                              if (setUpDone) {
                                 ...
                                 if (setUpDone) {
                                    log.info("Succesfully powered on the "
                                             + "VM " + this.vmName);
                                    if (!this.isVMCreated && checkGuest) {
    ...
    * @param connectAnchor ConnectAnchor object
    */
   @Test(description = "Convert a VM that connects to an early binding "
               + "DVPortgroup into template. The tempalte will "
               + "contain the information of the early binding "
               + "portgroup to connect to. Clone two VM's from the "
               + "template and power on the two VM's.")
      ...
      throws Exception
   {
      ...
      String ipAddress = null;
         ...
         this.ivm.markAsTemplate(this.vmMor);
         ...
         if (testDone) {
            vdConfigSpec = DVSUtil.getAllVirtualEthernetCardDevices(this.vmMor,
                     connectAnchor);
            if (vdConfigSpec != null && vdConfigSpec.size() > 0) {
               ...
               log.error("There are ethernet cards on the VM "
                        + this.vmName);
               testDone = false;
            }
            if (testDone) {
                ...
                */
               relocateSpec.setDatastore(this.ihs.getDatastoresInfo(
                        this.hostMor).get(0).getDatastoreMor());
