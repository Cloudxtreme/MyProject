 ...
 * to an late binding portgroup of the DVSwitch to a different datastore on a
 * host that has multiple datastores. The VM moves to the destination datastore.
 */
public class Pos004 extends SVMFunctionalTestBase
{

   public void setTestDescription()
   {
      setTestDescription("Storage vmotion a powered on VM via a vmotion virtual"
               + " nic that is connected to a latebinding portgroup of "
               + "the DVSwitch to a different datastore on a host "
               + "that has multiple datastores. The VM moves to the"
