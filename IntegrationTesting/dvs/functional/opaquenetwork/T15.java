...
public class T15 extends TestBase {
    ...
    public void initialize() throws Exception {
        ...
        //this.hostLoginSession = this.sessionManager.login(sessionMgrMor, "root", "ca$hc0w", null);
        //this.xvcProvisioningHelper = new XvcProvisioningHelper(this.connectAnchor, this.connectAnchor);
                   ...
                   throws Exception
                {
                   ...
                   if (hostFolderMor != null) {
                      ...
                      clusterMor = folder.createCluster(hostFolderMor, clusterName,
                               clusterSpec);
                      ManagedObjectReference[] a = new ManagedObjectReference[hostSystem.getAllHost().size()];
    ...
    public void cleanUpRelocate() throws Exception {
        ...
        //this.xvcProvisioningHelper = new XvcProvisioningHelper(this.connectAnchor, this.connectAnchor);
        //cleanupSpec.setService(this.xvcProvisioningHelper.getServiceLocator("root", "vmware"));
    ...
    public void registerVm() throws Exception {
        ...
        int count = 1;
        this.datastoreSystemMor = dataStoreSystem.getDatastoreSystem(this.destHostMor);
    ...
    public void unregisterVms() throws Exception {
        ...
        if (vmMorList != null && vmMorList.size() >= 1) {
            for (ManagedObjectReference vm_Mor : vmMorList) {
                assertTrue(
                    vm.setVMState(vm_Mor, VirtualMachinePowerState.POWERED_OFF, false),
    ...
    public void destroyVds() throws Exception {
        ...
        if (dcMor != null) {
            List<ManagedObjectReference> vdsMorList = folder.getAllDistributedVirtualSwitch(folder
    ...
    public void setSourceParameters() throws Exception {
        ...
        srcVdsMor = folder.createDistributedVirtualSwitch(folder.getNetworkFolder(dcMor), vdsConfigSpec);
        assertNotNull(srcVdsMor, "Successfully created the source vds", "Failed to create the source vds");
    }
    ...
    public void reconfigureVMConnectToVdsPort() throws Exception {
        ...
        DistributedVirtualSwitchPortConnection[] portConnection = new DistributedVirtualSwitchPortConnection[2];
        for (int i = 0; i < 2; i++) {
            portConnection[i] = new DistributedVirtualSwitchPortConnection();
            portConnection[i].setSwitchUuid(dvs.getConfig(srcVdsMor).getUuid());
        }
        ...
        Map<String, Map<String, Boolean>> ethernetCardMap = new HashMap<String, Map<String, Boolean>>();
        Map<String, String> ethMap = NetworkUtil.getEthernetCardNetworkMap(vmMor, connectAnchor);
        int i = 0;
        for (String dev : ethMap.keySet()) {
            Map<String, Boolean> portBoolMap = new HashMap<String, Boolean>();
        ...
        this.reconfigVmSpec = DVSUtil.reconfigureVMConnectToVdsPort(vmMor, connectAnchor, ethernetCardMap, dvs
            .getConfig(srcVdsMor).getUuid());
    }
    ...
    public boolean testSetUp() throws Exception {
         ...
         * Add a host to vds
         */
        List<ManagedObjectReference> dvsMorList = new ArrayList<ManagedObjectReference>();
        ...
        destVdsMor = this.folder.createDistributedVirtualSwitch(
            ...
            vdsConfigSpec);
        assertNotNull(destVdsMor, "Successfully created the destination vds", "Failed to create the destination vds");
        dvsMorList = new ArrayList<ManagedObjectReference>();
        dvsMorList.add(destVdsMor);
        assertTrue(
            DVSUtil.addSingleFreePnicAndHostToDVS(this.connectAnchor, this.destHostMor, dvsMorList),
            "Successfully added the host to " + "both the source and destination vdses",
            "Failed to add the " + "host to both the source and the destination vds");
        addPortgroups();
    ...
    public void test() throws Exception {
        ...
        //relocateSpec.setService(this.xvcProvisioningHelper.getServiceLocator("root", "vmware"));
        this.destDsMor = this.destDatastore.getDatastore(this.destHostMor, "vimapi_vms");
    ...
    public boolean testCleanUp() throws Exception {
            ...
            try {
                if(this.destVmMor != null){
                                if (destVm.getVMState(destVmMor).equals(VirtualMachinePowerState.POWERED_ON)) {
                            assertTrue(
