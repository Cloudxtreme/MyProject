...
/**
 * Convert a VM that connects to a late binding DVPortgroup into template. The
 * template will contain the portgroup information to connect to. Clone two VM's
 * from the template and power on the two VM's.
 */
public class Pos015 extends FunctionalTestBase
{
   ...
   {
      setTestDescription("Convert a VM that connects to a late binding "
               + "DVPortgroup into template. The template will contain"
               + " the portgroup information to connect to. Clone two"
               + " VM's from the template and power on the two VM's.");
   }
    ...
    * @param connectAnchor ConnectAnchor object
    */
   @Test(description = "Convert a VM that connects to a late binding "
               + "DVPortgroup into template. The template will contain"
               + " the portgroup information to connect to. Clone two"
               + " VM's from the template and power on the two VM's.")
   public void test()
      ...
      String ipAddress = null;
         ...
         if (testDone) {
            ...
            if (vdConfigSpec != null && vdConfigSpec.size() > 0) {
               ...
               testDone = false;
               log.error("There are no ethernet adapters on the VM "
                        + this.vmName);
            }
            if (testDone) {
               testDone = false;
