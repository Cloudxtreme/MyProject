...
/**
 * Suspend and resume a VM that is connected to a early binding portgroup of the
 * DVSwitch.
 */
public class Pos003 extends FunctionalTestBase
{
   ...
   public void setTestDescription()
   {
      setTestDescription("Suspend and resume a VM that is connected to a "
               + "early binding portgroup of the DVSwitch.");
   }
                           ...
                           if (pgMorList != null && pgMorList.size() == 1) {
                              ...
                              if (this.pgMor != null) {
                                 ...
                                 if (vmConfigSpec != null
                                    ...
                                    setUpDone = false;
                                    log.error("Can not get the VM config spec"
                                             + " to update to");
                                 }
                              } else {
                                 setUpDone = false;
      ...
      boolean checkGuest = DVSTestConstants.CHECK_GUEST;
         ...
         if (testDone) {
            ...
            if (usedPorts != null && usedPorts.size() > 0) {
               if (this.iDVS.refreshPortState(this.dvsMor,
                  ...
                  if (oldDVPorts != null) {
                     ...
                     if (testDone) {
                        ...
                        // statistics.
                        log.info("Sleeping for a sec to populate "
                        ...
                        if (testDone) {
                           if (this.iDVS.refreshPortState(
                              ...
                              if (newDVPorts != null
                                 ...
                                 testDone = false;
                                 log.error("Can not retrieve all the ports "
                                          + "used by the VM");
                              }
                           } else {
               ...
               testDone = false;
               log.error("Can not retrieve the ports used by the VM "
                        + this.vmName);
            }
         }
