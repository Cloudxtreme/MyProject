...
/**
 * Add a PVLAN map entry to the DVS with ID as 4095 with port type as
 * promiscuous.
 */
public class Neg023 extends PvlanBase
{
   /**
   ...
   {
      setTestDescription("Add a PVLAN map entry to the DVS with ID as 4095 "
               + "with port type as promiscuous.");
   }
    ...
    */
   @Override
   @Test(description = "Add a PVLAN map entry to the DVS with ID as 4095 "
               + "with port type as promiscuous.")
   public void test()
      throws Exception
   {
      ...
      try {
         iVmwareDVS.addPvlan(dvsMor, PVLAN_TYPE_PROMISCUOUS, MAX_PVLAN_ID + 1,
                  MAX_PVLAN_ID + 1);
