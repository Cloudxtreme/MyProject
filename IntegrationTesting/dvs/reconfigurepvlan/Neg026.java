...
/**
 * Add a PVLAN map entry to the DVS with primary ID as 4095 and secondary ID as
 * 1 with port type as isolated.
 */
public class Neg026 extends PvlanBase
{
   /**
   ...
   {
      setTestDescription("Add a PVLAN map entry to the DVS with primary ID as "
               + "4095 and secondary ID as 1 with port type as isolated.");
   }
    ...
    */
   @Override
   @Test(description = "Add a PVLAN map entry to the DVS with primary ID as "
               + "4095 and secondary ID as 1 with port type as isolated.")
   public void test()
      throws Exception
   {
      ...
      try {
         iVmwareDVS.addPvlan(dvsMor, PVLAN_TYPE_ISOLATED, MAX_PVLAN_ID + 1, 1);
