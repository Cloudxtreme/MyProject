 ...
 * DVPort. Build DVPortConnection with DVSwitchUuid as null. The device is of
 * type VirtualPCNet32,the backing is of type DVPort backing and the port
 * connection is a DVPort connection.
 */
public class Neg001 extends VMopsBase
{
   /*
    ...
    * Create the Standalone DVPort. 3. Create the DVPortConnection with
    * DVSwitchUuid as null. 4. Create the Default VMConfigSpec.
    *
   ...
   @Test(description = "Reconfigure a VM on a Standalone host to connect to"
               ...
               + " VirtualPCNet32.Build DVPortconnection with DVSUuid as null.")
   public void test()
      throws Exception
   {
      try {
         log.info("Enter the test method.");
         // Reconfigure a VM with DVSwitchUuid as null.
         VirtualMachineConfigSpec deltaConfigSpec = null;
         deltaConfigSpec = DVSUtil.getVMConfigSpecForDVSPort(vmMor,connectAnchor,
                  new DistributedVirtualSwitchPortConnection[]
                  {dvsPortConnection })[0];
         if (ivm.reconfigVM(vmMor, deltaConfigSpec)) {
            log.error("Successfully reconfigure the VM.API didn't throw any " +
               "exception.");
         }
