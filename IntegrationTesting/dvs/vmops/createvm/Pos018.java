...
/**
 * Create 2 VM's on a standalone host to connect to an existing lateBinding
 * DVPortgroup containing 1 unused port.The device is of type VirtualVmxNet2.
...
public class Pos018 extends VMopsBase
{
   ...
   {
      setTestDescription("Create 2 VM's on a Standalone host to connect to"
               + "an existing lateBinding DVPortgroup containing 1 unused port."
               + "The device is of type VirtualVmxNet2.");
   }

   /**
    * Method to setup the environment for the test. 1. Create the DVSwitch. 2.
    * Create the lateBinding DVPortgroup,it has 4 unused ports. 3. Create the VM
    * ConfigSpec.
    * 
      ...
      throws Exception
   {
      ...
      if (super.testSetUp()) {
            ...
            if (hostNetworkConfig[0] != null && hostNetworkConfig[1] != null) {
               ...
               if (networkUpdated) {
                  // Get the lateBinding portgroupKey.
   ...
   /**
    * Test. 1. Create the VM's. 2. Varify the ConfigSpecs and Power-ops
    * operations.
    * 
    * @param connectAnchor ConnectAnchor.
    */
   @Override
   @Test(description = "Create 2 VM's on a Standalone host to connect to"
               + "an existing lateBinding DVPortgroup containing 1 unused port."
               + "The device is of type VirtualVmxNet2.")
   public void test()
      throws Exception
   {
      boolean status = false;
         ...
         vmMor1 = new Folder(super.getConnectAnchor()).createVM(
                  ...
                  hostMor);
         if (vmMor1 != null) {
            log.info("Successfully create the first VM.");
            status = verify(vmMor1, null, vmConfigSpec);
            if (status) {
               ...
               if (vmMor2 != null) {
                  log.info("Successfully create the second VM.");
                  status = verify(vmMor2, null, vmConfigSpec);
                  assertTrue(status, "Test Failed");
               } else {
                  log.error("Failed to create the second VM.");
                  status = false;
               }
            }
         } else {
            log.error("Failed to create the first VM.");
         }
