...
public class Pos010 extends CreateDVSTestBase
{
   ...
   public void setTestDescription()
   {
      super.setTestDescription("Reconfigure DVPort and set a valid "
               + "datacenter ComputerResource Mor in scope property");
   }
            ...
            if (this.networkFolderMor != null) {
               ...
               if (this.dvsMOR != null) {
                  ...
                  if (portKeyList != null && portKeyList.size() == DVS_PORT_NUM) {
                     ...
                     ManagedObjectReference dcMor = ihs.getDataCenter();
                     if (dcMor != null) {
                        ...
                        portConfigSpecs[0].getScope().addAll(com.vmware.vcqa.util.TestUtil.arrayToVector(new ManagedObjectReference[] { dcMor }));
                        portConfigSpecs[0].setOperation(ConfigSpecOperation.EDIT.value());
                        status = true;
                     } else {
                        log.error("Can't get host in inventory");
                     }
                  } else {
