...
/**
 * Add two early binding portgroups pg1, and pg2 to the DVSwitch with both the
 * portgoups having the same promiscous pvlan id in the default port config.
 */
public class Pos050 extends TestBase
{
   ...
   public void setTestDescription()
   {
      setTestDescription("Add two early binding portgroups pg1, and pg2 to "
               + "the DVSwitch with both the portgoups having the"
               + " same promiscous pvlan id in the default port " + "config.");
   }
                  ...
                  if (this.hostNetworkConfig != null
                     ...
                     if (this.ins.updateNetworkConfig(this.nwSystemMor,
                        ...
                        if (this.iDVSwitch.addPvlan(this.dvsMor,
                                 DVSTestConstants.PVLAN_TYPE_PROMISCUOUS,
                                 PROMISCUOUS_PVLAN_ID, PROMISCUOUS_PVLAN_ID)) {
      ...
      ManagedObjectReference vmMor = null;
         ...
         if (this.dvPortgroupMorList != null) {
            for (ManagedObjectReference mor : dvPortgroupMorList) {
               status &= this.iDistributedVirtualSwitch.destroy(mor);
            }
         }
         if (this.dvsMor != null) {
