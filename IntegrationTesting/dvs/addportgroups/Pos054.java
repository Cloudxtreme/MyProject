...
/**
 * Add two early binding portgroups pg1, and pg2 to the DVSwitch with pg1 having
 * a community pvlan id in the default port config and pg2 having a different
 * community pvlan id in the default port config.
 */
public class Pos054 extends TestBase
{
   ...
   private static final int COMMUNITY_PVLAN_ID_1 = 20;
   private static final int COMMUNITY_PVLAN_ID_2 = 30;
   ...
   {
      setTestDescription("Add two early binding portgroups pg1, and pg2 to"
               + " the DVSwitch with pg1 having a community pvlan"
               + " id in the default port config and pg2 having a"
               + " different community pvlan id in the default port"
               + " config.");
   }
                  ...
                  if (this.hostNetworkConfig != null
                     ...
                     if (this.ins.updateNetworkConfig(this.nwSystemMor,
                        ...
                        if (this.iDVSwitch.addSecondaryPvlan(this.dvsMor,
                                 ...
                                 PROMISCUOUS_PVLAN_ID, COMMUNITY_PVLAN_ID_1,
                                 true)) {
                           log.info("Successfully added the pvlan id to "
                                    + "the DVSwitch");
                           if (this.iDVSwitch.addSecondaryPvlan(this.dvsMor,
                                    DVSTestConstants.PVLAN_TYPE_COMMINITY,
                                    PROMISCUOUS_PVLAN_ID, COMMUNITY_PVLAN_ID_2,
                                    false)) {
                              pvlanSpec = new VmwareDistributedVirtualSwitchPvlanSpec();
                              pvlanSpec.setPvlanId(COMMUNITY_PVLAN_ID_1);
                              settingsMap = new HashMap<String, Object>();
                              settingsMap.put(DVSTestConstants.VLAN_KEY,
                                       pvlanSpec);
                              ...
                              portgroupConfigSpec.setName(this.getTestId()
                                       + "-pg1");
                              portgroupConfigSpec.setNumPorts(1);
                              portgroupConfigSpec.setType(DVSTestConstants.DVPORTGROUP_TYPE_EARLY_BINDING);
                              ...
                              portgroupConfigSpec.setType(DVSTestConstants.DVPORTGROUP_TYPE_EARLY_BINDING);
                              pvlanSpec = new VmwareDistributedVirtualSwitchPvlanSpec();
                              pvlanSpec.setPvlanId(COMMUNITY_PVLAN_ID_2);
                              settingsMap.put(DVSTestConstants.VLAN_KEY,
                                       pvlanSpec);
                              portSetting = DVSUtil.getDefaultVMwareDVSPortSetting(settingsMap);
                              portgroupConfigSpec.setDefaultPortConfig(portSetting);
    ...
    * @param connectAnchor ConnectAnchor object
    */
   @Test(description = "Add two early binding portgroups pg1, and pg2 to"
               + " the DVSwitch with pg1 having a community pvlan"
               + " id in the default port config and pg2 having a"
               + " different community pvlan id in the default port"
               + " config.")
      ...
      throws Exception
   {
      ...
      VirtualMachineConfigSpec[] vmConfigSpec = null;
         ...
         if (this.dvPortgroupMorList != null
                  && this.dvPortgroupMorList.size() == 2
                  && this.dvPortgroupMorList.get(0) != null
                  && this.dvPortgroupMorList.get(1) != null) {
            portkeys = this.iDVPortgroup.getPortKeys(this.dvPortgroupMorList.get(0));
            if (portkeys != null && portkeys.size() == 1) {
               portConnection = new DistributedVirtualSwitchPortConnection();
               portConnection.setSwitchUuid(this.iDVSwitch.getConfig(
                        this.dvsMor).getUuid());
               portConnection.setPortgroupKey(this.iDVPortgroup.getConfigInfo(
               ...
               originalVMConfigSpec = DVSUtil.configureVM(connectAnchor,
                        vm1Mor, this.hostMor, portConnection);
               ...
               portkeys = this.iDVPortgroup.getPortKeys(this.dvPortgroupMorList.get(1));
               portConnection.setPortgroupKey(this.iDVPortgroup.getConfigInfo(
                        this.dvPortgroupMorList.get(1)).getKey());
               portConnection.setPortKey(portkeys.get(0));
