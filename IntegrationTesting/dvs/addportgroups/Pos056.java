 ...
 * an community pvlan id in the default port config and pg2 having an isolated
 * pvlan id in the default port config.
 */
public class Pos056 extends TestBase
{
   ...
   private static final int ISOLATED_PVLAN_ID = 30;
   ...
   public void setTestDescription()
   {
      setTestDescription("Add two early binding portgroups pg1, and pg2 to "
               + "the DVSwitch with pg1 having an community pvlan"
               + " id in the default port config and pg2 having an"
               + " isolated pvlan id in the default port config.");
   }
                  ...
                  if (this.hostNetworkConfig != null
                     ...
                     if (this.ins.updateNetworkConfig(this.nwSystemMor,
                        ...
                        if (this.iDVSwitch.addSecondaryPvlan(this.dvsMor,
                                 DVSTestConstants.PVLAN_TYPE_ISOLATED,
                                 PROMISCUOUS_PVLAN_ID, ISOLATED_PVLAN_ID, true)) {
                           log.info("Successfully added the pvlan id to "
                                    + "the DVSwitch");
                           if (this.iDVSwitch.addSecondaryPvlan(this.dvsMor,
                                    DVSTestConstants.PVLAN_TYPE_COMMINITY,
                                    PROMISCUOUS_PVLAN_ID, COMMUNITY_PVLAN_ID,
                                    false)) {
                              pvlanSpec = new VmwareDistributedVirtualSwitchPvlanSpec();
                              pvlanSpec.setPvlanId(ISOLATED_PVLAN_ID);
