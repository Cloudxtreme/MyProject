 ...
 * set in the configSpec: - DVSConfigSpec.maxPort set to a valid number more
 * than the numPorts - DVSConfigSpec.numPort set to a valid number more than the
 * number of uplinks ports per host - DVSConfigSpec.uplinkPortPolicy set to a
 * valid array that is equal to the the number of pnics being added({uplink1, ..
 * uplinkn}). - DistributedVirtualSwitchHostMemberConfigSpec.operation set to
 * add - DistributedVirtualSwitchHostMemberConfigSpec.host set to a valid host
 * Mor - DistributedVirtualSwitchHostMemberConfigSpec.proxy set a a valid pnic
 ...
 */
public class Pos024 extends CreateDVSTestBase
{
   /*
   ...
   public void setTestDescription()
   {
      super.setTestDescription("Create DVSwitch with the following parameters"
               ...
               + "- DVSConfigSpec.maxPort set to a valid number more than the numPorts\n"
               + "- DVSConfigSpec.numPort set to a valid number more than the number of"
               + "uplinks ports per host\n"
               + "- DVSConfigSpec.uplinkPortPolicy set to a valid array that is equal to"
               + " the number of pnics being added({uplink1, .. uplinkn}),\n"
         ...
         if (super.testSetUp()) {
            ...
            if (this.networkFolderMor != null) {
               ...
               if (physicalNics != null) {
                  ...
                  uplinkPortNames = new String[physicalNics.length];
                  for (int i = 0; i < physicalNics.length; i++) {
                     uplinkPortNames[i] = "uplink" + i;
                  }
                  this.configSpec.setMaxPorts(2 * uplinkPortNames.length + 5);
                  this.configSpec.setNumStandalonePorts(uplinkPortNames.length + 2);
