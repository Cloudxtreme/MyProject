...
sub STAF::STAFResult::new
{
    if ($#_ < 2)
    {
        die "Usage: STAFResult->new(RC, RESULT)\n";
    }
...
sub STAF::STAFUnmarshall
{
    ...
    if (substr($data, 0, length(NONE_MARKER)) eq NONE_MARKER)
    {
    ...
    elsif (substr($data, 0, length(SCALAR_MARKER)) eq SCALAR_MARKER)
    {
        ...
        $colonIndex = index($data, ':', $colonIndex + 1);

        my $theString = substr($data, $colonIndex + 1);
    ...
    elsif (substr($data, 0, length(MC_INSTANCE_MARKER)) eq MC_INSTANCE_MARKER)
    {
        ...
        # Skip past map-class-instance-length and then get the map class name

        my $colonIndex = index($data, ':');
        my $colonIndex2 = index($data, ':', $colonIndex + 1);

        $colonIndex = index($data, ':', $colonIndex2 + 1);
        ...
        my $dataIndex = $colonIndex2 + 1 + $mapClassNameLength;
    ...
    elsif (substr($data, 0, length(CONTEXT_MARKER)) eq CONTEXT_MARKER)
    {
        ...
        my $contextLengthIndex = index($data, ':') + 1;
        my $contextIndex = index($data, ':', $contextLengthIndex) + 1;
        my $contextLength = substr($data, $contextLengthIndex,
                                   $contextIndex - 1 - $contextLengthIndex);

        my $colonIndex = index($data, ':', $contextIndex);
        my $mapIndex = $contextIndex;
        my $mapDataIndex = index($data, ':', $colonIndex + 1) + 1;
        my $mapLength = substr($data, $colonIndex + 1,
                               $mapDataIndex - $colonIndex - 2);
        ...
        $context->setRootObject(
            STAF::STAFUnmarshall(
                substr($data, $rootObjIndex,
                       $rootObjDataIndex + $rootObjLength - $rootObjIndex + 1),
...
use constant kServiceAlreadyExists        => scalar 40;
