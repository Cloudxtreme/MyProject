...
class NSX70SwitchImpl(switch_interface.SwitchInterface):
    ...
    @classmethod
    def get_connection_table(cls, client_object, get_connection_table=None,
                             switch_vni=None, endpoints=None):
        """
        Gets the connection entries related to a VNI from the master
        controller.

        We expect that there will be only one master controller responsible
        for VNI connection entries and if product reflects that more
        than one controller is responsible for VNI, then that is raised as
        an error since that is a product bug.

        @type client_object: LogicalSwitchCLIClient
        @param client_object: Client object (TBD: might be unused here).
        @type switch_vni: int
        @param switch_vni: VNI for the switch.
        @type get_connection_table: list
        @param get_connection_table: A list of host IPs that are connected to
            the given switch_vni.
        @type endpoints: list
        @rtpe: ConnectionTableSchema
        """
        # The endpoints are used for querying the state on those
        # objects and logical switch's CLI client is just a dummy client.
        _ = client_object, get_connection_table
        queried_object = cls._get_query_object(endpoints,
                                               switch_vni)
        return queried_object.get_connection_table(switch_vni=switch_vni)
        ...
        if len(endpoints) > 1:
            ...
            if not master_controllers:
                ...
                raise errors.CLIError(
                    status_code=common.status_codes.NOT_FOUND,
                    reason=reason)
            if len(master_controllers) > 1:
                raise AssertionError('More than one controllers [%r] have '
                                     ...
                                     ([mc.ip for mc in master_controllers],
                                      switch_vni))
    ...
    def get_vtep_table(cls, client_object, get_vtep_table=None,
        ...
        @type endpoints: list
        @param endpoints: Host object(s) which implement the
            interface for retrieving the Vtep table.
        @rtpe: VtepTableSchema
        """
        # The endpoints are used for querying the state on those
        # objects and logical switch's CLI client is just a dummy client.
        _ = client_object
    ...
    @classmethod
    def read_switch_ccp_mapping(cls, client_object, endpoints=None,
                                switch_vni=None):
        """
        Gets the master ccp node based on given logcial switch vni

        For controller objects, we expect that there will be only one master
        controller responsible for one logical switch and if product reflects
        that more than one controller is responsible for VNI, then that is
        raised as an error since that is a product bug.
        ...
        @param switch_vni: VNI for the switch.
        @type endpoints: list
        @param endpoints: CCP object(s) which implement the
            interface for retrieving the stats table.
        """
        # The endpoints are used for querying the state on those
        # objects and logical switch's CLI client is just a dummy client.
