class ListBindingsParser:
    ...
    def get_parsed_data(self, input, delimiter=' '):
        ...
        for line in data:
            pydict = dict()
            value = line.split()
            pydict.update({'source_kind':value[0]})
            pydict.update({'source_name':value[1]})
            pydict.update({'destination_kind':value[2]})
            pydict.update({'destination_name':value[3]})
