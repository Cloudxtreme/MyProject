...
/**
 * DESCRIPTION:Associate/Disassociate a newly created nrp with multiple dvPort<BR>
 * TARGET: VC <BR>
 * SETUP:<BR>
 * 1. Create a vDS<BR>
 * 2. Enable netiorm <BR>
 * 3. Add a nrp to it<BR>
 * 4. Create multiple dv port<BR>
 * TEST:<BR>
 * 5. Reconfigure multiple port on the dvs with nrp associated to it<BR>
 * 6. Verify that the ports are associated with the nrp<BR>
 * CLEANUP:<BR>
...
public class Pos062 extends TestBase
{
      ...
      throws Exception
   {
      ...
      // set the number of ports to 4
      dvpgSpec[0].setNumPorts(4);
    ...
    * Test method
    */
   @Test(description = "Associate/Disassociate a newly created nrp with multiple dvPort")
   public void test()
      throws Exception
   {
      // Associate the nrp to the dvport specs
      NetworkResourcePoolHelper.associateNrpToDvportSpec(dvPortSpec, nrp);
      // reconfigure the dvport
      Assert.assertTrue(dvs.reconfigurePort(dvsMor, dvPortSpec),
               "Successfully reconfigured dvports with nrp attached",
   ...
   {
      setTestDescription("Associate/Disassociate a newly created nrp with multiple dvPort");
   }
