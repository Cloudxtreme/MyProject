 ...
 */
public class Pos013 extends CreateDVSTestBase
{
   ...
   private ManagedObjectReference resPoolMor = null;
   ...
   public void setTestDescription()
   {
      super.setTestDescription("Reconfigure DVPort and set a valid "
               + "nested ResourcePool Mor in scope property");
   }
            ...
            if (this.networkFolderMor != null) {
               ...
               if (this.dvsMOR != null) {
                  ...
                  if (portKeyList != null && portKeyList.size() == DVS_PORT_NUM) {
                     ...
                     if (hostMor != null) {
                        ManagedObjectReference rpMor = icr.getResourcePool(
                                 ihs.getParentNode(hostMor));
                        this.resPoolMor = this.createChildResPool(rpMor);
                        if (resPoolMor != null) {
                           ...
                           portConfigSpecs[0].getScope().addAll(com.vmware.vcqa.util.TestUtil.arrayToVector(new ManagedObjectReference[] { this.resPoolMor }));
                           portConfigSpecs[0].setOperation(ConfigSpecOperation.EDIT.value());
                           status = true;
                        } else {
                           log.error("Can't create new resource pool ");
                        }
                     } else {
                        log.error("Cannot find a standalone host in inventory");
                     }
                  } else {
      ...
      throws Exception
   {
      boolean status = true;
         ...
         if (this.resPoolMor != null) {
            if (this.irp.destroy(this.resPoolMor)) {
