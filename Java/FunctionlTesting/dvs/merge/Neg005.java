...
/**
 * Merge a destination DVS with valid MOR with a source DVS which has a folder
 * MOR
 */
public class Neg005 extends TestBase
{
   ...
   public void setTestDescription()
   {
      setTestDescription("Merge a destination DVS with valid MOR with a "
               + "source DVS which has folder MOR");
   }
      ...
      throws Exception
   {
      ...
      boolean status = false;
      try {
         if (this.iDVSwitch.merge(destDvsMor, rootFolder)) {
         ...
         MethodFault expectedMethodFault = new InvalidType();
