...
/**
 * Create two early binding portgroups (each having 2 ports) having the
 * different names with two VMvnics connected to each different portgroups
 * having different portNameFormat and no conflicting ports. Merge the switches
 */
public class Pos021 extends TestBase
{
   ...
   {
      setTestDescription("Create two early binding portgroups (each having "
    ...
    * @param connectAnchor ConnectAnchor object
    */
   @Test(description = "Create two early binding portgroups (each having "
               + "2 ports) having the different names with two VMvnics connected to "
               + "each different portgroups having different portNameFormat and "
               + "no conflicting ports. Merge the switches")
   public void test()
      ...
      boolean status = false;
         ...
         if (this.iDVSwitch.merge(destDvsMor, srcDvsMor)) {
            ...
            if (this.iNetworkSystem.refresh(this.secondNetworkMor)) {
               log.info("Successfully refreshed the host network system "
      ...
      boolean status = true;
         ...
         if (this.destfolder != null) {
            if (this.iFolder.destroy(this.destfolder)) {
               log.info("Successfully destroyed the destination DVS folder");
            } else {
               log.error("Can not destroy the destination DVS folder");
            }
         }
         if (this.srcfolder != null) {
            if (this.iFolder.destroy(this.srcfolder)) {
               log.info("Successfully destroyed the source DVS folder");
            } else {
               log.error("Can not destroy the source DVS folder");
            }
         }
