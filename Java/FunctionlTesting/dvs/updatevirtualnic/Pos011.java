 ...
 * Update the scsiName using valid Name and valid HbaId
 */
public class Pos011 extends VNicBase
{
   ...
   private String orgIscsiName = null;
   private String newIscsiName = null;
   private boolean isScsiNameChanged = false;
   ...
   {
      setTestDescription("Update an existing  virtual  nics  to"
               + " connect to an standalone port on an existing "
               + "DVSwitch. "
                                 ...
                                 if (hbas != null) {
                                    for (int i = 0; i < hbas.length; i++) {
                                       if (hbas[i] instanceof HostInternetScsiHba) {
                                          ...
                                          orgIscsiName = ((HostInternetScsiHba) hbas[i]).getIScsiName();
                                          newIscsiName = "iqn.2000-04.com.vmware:soft.iscsisdk202";
    ...
    */
   @Override
   @Test(description = "Update an existing  virtual  nics  to"
               + " connect to an standalone port on an existing "
               + "DVSwitch. "
               + "The distributedVirtualPort is of type DVSPortConnection.\n"
               + "Update the scsiName using valid Name and valid HbaId ")
      ...
      log.info("test setup Begin:");
         ...
         if (portConnection != null) {
            ...
            if (ins.updateVirtualNic(nwSystemMor, vNicdevice, updatedVNicSpec)) {
               ...
               if (iscsi.updateInternetScsiName(storageSystemMor, iscsiHbaId,
                        newIscsiName)) {
                  log.info("iScsiName for Hba Id " + iscsiHbaId
                           + " changed sucessfully to " + newIscsiName);
                  isScsiNameChanged = true;
                  status = true;
               } else {
                  log.info("iScsiName for Hba Id " + iscsiHbaId
      ...
      throws Exception
   {
      boolean status = false;
         ...
         try {
            if (isScsiNameChanged) {
               if (iscsi.updateInternetScsiName(storageSystemMor, iscsiHbaId,
                        orgIscsiName)) {
                  log.info("ScsiName change success");
                  status = true;
               } else {
                  log.info("ScsiName change fail");
               }
            }
         } catch (Exception ex) {
