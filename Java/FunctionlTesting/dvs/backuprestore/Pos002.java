 ...
 * vNics (TODO: currently reconfig VM is failing, need to fix it) 4. Define
 * DVPG1 as the backing for vNic1 and DVPG2 as the backing for vNic2 5. Power on
 * VM 6. Power OFF VM
 */
public class Pos002 extends TestBase {
	...
	EntityBackupConfig[] vDSPGConfig = null;

	private Vector<ManagedObjectReference> dvsMorList = new Vector<ManagedObjectReference>(
			2);
	private Vector<ManagedObjectReference> dvpgMorList = new Vector<ManagedObjectReference>(
			2);
	...
	public void setTestDescription() {
		super.setTestDescription("Test export functionality with port groups alone\n"
				+ " 1. Create DVS'es as many as the free pNICS available\n"
				+ " 2. export the port group configuration alone"
				+ " 3. Delete the port groups and import the same from the saved config\n"
				+ " 4. Verify the port groups are re-created\n");
	}
	 ...
	 */
	@Override
	@Test(description = "Test export functionality with port groups alone\n"
			+ " 1. Create DVS'es as many as the free pNICS available\n"
			+ " 2. export the port group configuration alone"
			+ " 3. Delete the port groups and import the same from the saved config\n"
			+ " 4. Verify the port groups are re-created\n")
	public void test() throws Exception {
		...
		try {
			...
			// Create as many DVS'es as the available pNICs
			selectionSet = new SelectionSet[freePnics.length];
			for (int pniccnt = 0; pniccnt < freePnics.length; pniccnt++) {
				dvsMor = migrateNetworkToDVS(hostMor, freePnics[pniccnt], "DVS"
						+ pniccnt);
            ...
            dvpgSS.getPortgroupKey().addAll(com.vmware.vcqa.util.TestUtil.arrayToVector(ephemeral));
				...
				assertNotNull(ephemeral[0], "Unable to create ephemeral PG");
				portConnection = new DistributedVirtualSwitchPortConnection();
				portConnection.setSwitchUuid(dvsUUID);
				portConnection.setPortgroupKey(dvspgkey);
				pcs.add(portConnection);
				dvpgMorList.add(ephepg);
			}

			vDSPGConfig = iDVSMgr.exportEntity(dvsManagerMor, selectionSet);
			// Iterate through the export configuration, delete the port group
			// and recreate it using the importEntity API
			for (int configIndex = 0; configIndex < vDSPGConfig.length; configIndex++) {
				srcDVSMor = vDSPGConfig[configIndex].getContainer();
				vDSPGConfig[configIndex].setKey(iDistributedVirtualSwitch
						.getConfig(srcDVSMor).getUuid());
			}
			// The below for loop should go inside the above for loop
			for (ManagedObjectReference pgMor : dvpgMorList) {
				idvpg.destroy(pgMor);
			}

			iDVSMgr.importEntity(dvsManagerMor, vDSPGConfig,
