 ...
 * will be modified as necessary later 1. Create 2 MN version DVS'es (DVS1 and
 * DVS2)\n) 2. create ephemeral port groups DVS1PG on DVS1 and DVS2PG on DVS2 3.
 * create a VM with 2 vNics 4. Define DVPG1 as the backing for vNic1 and DVPG2
 * as the backing for vNic2 5. Power on VM 6. Power OFF VM
 */
public class Pos005 extends TestBase {
	...
	public void setTestDescription() {
		super.setTestDescription("Test Export/Import operation of MN VDS port groups on MN.next host\n"
				+ " 1. Create 2 MN version DVS'es (DVS1 and DVS2)\n"
...

	{
		...
		try {
			...
			if (allHosts != null) {
				int count = 1;
				for (final ManagedObjectReference hostMor : allHosts) {
					...
					if (nsMor != null) {
						...
						if (pnicIds != null) {
							...
							vmConfigSpec = buildDefaultSpec(
									...
									vmName, 1);
							vmMor = new Folder(super.getConnectAnchor())
									.createVM(ivm.getVMFolder(), vmConfigSpec,
	 ...
	 */
	@Override
	@Test(description = "Test Export/Import operation of MN VDS port groups on MN.next host\n"
			+ " 1. Create 2 MN version DVS'es (DVS1 and DVS2)\n"
			+ " 2. create ephemeral portgroups DVPG1 on DVS1 and DVPG2 on DVS2\n"
			+ " 3. create a VM with 2 vNics\n"
			+ " 4. Define DVPG1 as the backing for vNic1 and DVPG2 as the backing for vNic2\n"
		...
		try {
			...
			if (freePnics != null && freePnics.length >= 2) {
               ...
               dvpgSS.getPortgroupKey().addAll(com.vmware.vcqa.util.TestUtil.arrayToVector(ephemeral));
				...
				vDSConfig[1].setName("importDVS1pg");
				vDSConfig[2].setName("importDVS2");
				vDSConfig[3].setName("importDVS2pg");
	...
	private ManagedObjectReference migrateNetworkToDVS(
      ...
      pnicBacking.getPnicSpec()
		...
		dvsConfigSpec.setConfigVersion("5.0.0");
