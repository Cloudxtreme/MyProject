...
public class Pos008 extends TestBase {
	...
	EntityBackupConfig[] dvsConfig = new EntityBackupConfig[2];
	EntityBackupConfig[] dvpgConfig = new EntityBackupConfig[2];
	EntityBackupConfig[] Pos008VDSConfig = null;
	...
	public void setTestDescription() {
		super.setTestDescription("PR# 426187  : test case \n"
	 ...
	 */
	@Override
	@Test(description = "PR# 426187  : test case \n"
			+ " 1. Create 2 DVS (DVS1 and DVS2)\n"
			+ " 2. create ephemeral portgroups DVPG1 on DVS1 and DVPG2 on DVS2\n"
			+ " 3. create a VM with 2 vNics\n"
			+ " 4. Define DVPG1 as the backing for vNic1 and DVPG2 as the backing for vNic2\n"
	...
	public void test() throws Exception {
		...
		int count = 0;
		String[] dvs1pgkey = new String[1];
		String[] dvs2pgkey = new String[1];
		String dvsUUID;
		String dvspgkey;
		...
		try {
			...
			if (freePnics != null && freePnics.length >= 2) {
				...
				for (ManagedObjectReference mor : dvsMorList) {
					...
					dvspgkey = idvpg.getKey(ephepg);
					String[] ephemeral = new String[1];
					ephemeral[0] = dvspgkey;
               ...
               dvpgSS.getPortgroupKey().addAll(com.vmware.vcqa.util.TestUtil.arrayToVector(ephemeral));
					...
					assertNotNull(ephepg, "Unable to create ephemeral PG");
				...
				/*
				 * While restoring the configuration from the backup, based on
				 * the import type and the type of the selection set that is
				 * being restoring, it could be a multi-step process. In this
				 * scenario, we need to first restore the DVS and then restore
				 * port groups
				 */

				Pos008VDSConfig = iDVSMgr.exportEntity(dvsManagerMor,
						selectionSet);
				for (int i = 0, dvscnt = 0, dvspgcnt = 0; i < Pos008VDSConfig.length; i++) {
					log.info("the entity type of config at " + i + "is "
							+ Pos008VDSConfig[i].getEntityType());
					if (Pos008VDSConfig[i].getEntityType().equals(
							"distributedVirtualSwitch")) {
						dvsConfig[dvscnt++] = Pos008VDSConfig[i];
					}
					if (Pos008VDSConfig[i].getEntityType().equals(
							"distributedVirtualPortgroup")) {
						dvpgConfig[dvspgcnt++] = Pos008VDSConfig[i];
					}
				}
				dvsConfig[0].setName("importDVS1");
				dvsConfig[1].setName("importDVS2");
				iDVSMgr.importEntity(dvsManagerMor, dvsConfig,
						"createEntityWithNewIdentifier");
				dvpgConfig[0].setName("importDVS1pg");
				dvpgConfig[1].setName("importDVS2pg");

				dvpgConfig[0].setContainer(iFolder.getDistributedVirtualSwitch(
						iFolder.getNetworkFolder(dcMor), "importDVS1"));
				dvpgConfig[1].setContainer(iFolder.getDistributedVirtualSwitch(
						iFolder.getNetworkFolder(dcMor), "importDVS2"));
