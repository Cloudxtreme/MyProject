...
public class Neg009 extends TestBase
{
   ...
   {
      super.setTestDescription("import the saved configuration while the same config exists\n"
               + " 1. Create 2 DVS (DVS1 and DVS2)\n"
   ...
   @Test(description = "import the saved configuration while the same config exists\n"
               + " 1. Create 2 DVS (DVS1 and DVS2)\n"
               + " 2. create ephemeral portgroups DVPG1 on DVS1 and DVPG2 on DVS2\n"
               + " 3. create a VM with 2 vNics\n"
               + " 4. Define DVPG1 as the backing for vNic1 and DVPG2 as the backing for vNic2\n"
      ...
      try {
         ...
         try {
            ...
            if (freePnics != null && freePnics.length >= 2) {
               ...
               assertTrue((iNetworkSystem.refresh(iNetworkSystem.getNetworkSystem(hostMor))),
               ...
               for (ManagedObjectReference mor : dvsMorList) {
                  ...
                  assertNotNull(dvspgkey, "Unable to create ephemeral PG");
               ...
               assertNotNull(vDSConfig, "Unable to export the given vDS " +
               		"configuration");
