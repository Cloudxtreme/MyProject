...
/**
 * DESCRIPTION:<br>
 * Edit a VSPAN session in a DVS without providing the VSPAN key.
 */
public class Neg011 extends VspanTestBase
{
   ...
   {
      setTestDescription("Edit a VSPAN session in a DVS without providing "
               + "the VSPAN key.");
   }
   ...
   {
      initialize();
      dvsMor = folder.createDistributedVirtualSwitch(dvsName,hs.getConnectedHost(false));
      vspanCfg = new VMwareDVSVspanConfigSpec[1];
      final VMwareVspanPort sourcePortTx;
      setupPortgroups(dvsMor);
      port = VspanHelper.popPort(portGroups);
   ...
   @Test(description = "Edit a VSPAN session in a DVS without providing "
               + "the VSPAN key.")
   @Override
   public void test()
   throws Exception
