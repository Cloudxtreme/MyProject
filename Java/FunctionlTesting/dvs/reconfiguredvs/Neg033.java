 ...
 * DistributedVirtualSwitchHostMemberConfigSpec.proxy to an invalid pnic proxy
 * selection that has multiple pnicSpecs having one invalid pnic key.
 */

public class Neg033 extends CreateDVSTestBase
{
   /*
   ...
   public void setTestDescription()
   {
      super.setTestDescription("Reconfigure an existing DVSwitch by setting:\n "
               ...
               + "that multiple pnicSpecs having one invalid pnic key.");
   }
         ...
         if (super.testSetUp()) {
            ...
            if (this.networkFolderMor != null) {
               ...
               if (this.dvsMOR != null) {
                  ...
                  DistributedVirtualSwitchHostMemberPnicSpec[] pnicSpec = new DistributedVirtualSwitchHostMemberPnicSpec[4];
                  DistributedVirtualSwitchHostMemberPnicBacking pnicBacking = new DistributedVirtualSwitchHostMemberPnicBacking();
                  for (int i = 0; i < 4; i++) {
                     pnicSpec[i] = new DistributedVirtualSwitchHostMemberPnicSpec();
                     pnicSpec[i].setPnicDevice("invalidPnicKey");
                  }
                  pnicBacking.getPnicSpec().clear();
                  pnicBacking.getPnicSpec().addAll(com.vmware.vcqa.util.TestUtil.arrayToVector(pnicSpec));
