 ...
 * reconfigure a VM to connect its VNIC to this portgroup with blocked property
 * set to true
 */
public class Pos027 extends TestBase
{
            ...
            if (this.rootFolderMor != null) {
               ...
               if (dvsMor != null) {
                  ...
                  if (nsMor != null) {
                      ...
                      */
                     if (allVMs != null) {
                        this.vmMor = getVM(hostMor, connectAnchor);
                     }
                     if (this.vmMor != null) {
                        ...
                        if (this.iVirtualMachine.setVMState(vmMor, VirtualMachinePowerState.POWERED_OFF, false)) {
                           ...
                           settingsMap.put(DVSTestConstants.BLOCKED_KEY, true);
      ...
      throws Exception
   {
      log.info("Test Begin:");
      boolean status = false;
      int numEthernetCards = DVSUtil.getAllVirtualEthernetCardDevices(vmMor,
               connectAnchor).size();
      DistributedVirtualSwitchPortConnection[] portConn = new DistributedVirtualSwitchPortConnection[numEthernetCards];
            ...
            if (dvPortgroupMorList != null) {
               if (dvPortgroupMorList.size() == dvPortgroupConfigSpecArray.length) {
                  ...
                  if (portgroupKey != null) {
                      ...
                      */
                     for (int i = 0; i < numEthernetCards; i++) {
                        portConn[i] = portConnection;
                     }
                     vmDeltaConfigSpec = DVSUtil.getVMConfigSpecForDVSPort(
                              vmMor, connectAnchor, portConn);
                     if (vmDeltaConfigSpec != null) {
                        if (this.iVirtualMachine.reconfigVM(vmMor,
                           ...
                           if (this.iVirtualMachine.setVMState(vmMor, VirtualMachinePowerState.POWERED_ON, true)) {
                              if (this.iVirtualMachine.getIPAddress(vmMor) == null) {
                                 status = true;
                              } else {
   ...
   /**
    * Returns the VirtualMachine with one ethernet adapter
    * 
    * @param hostMor Host Mor
    * @param connectAnchor ConnectAnchor object
    * @return VirtualMachine
    * @throws MethodFault, Exception
    */
   private ManagedObjectReference getVM(ManagedObjectReference hostMor,
                                        ConnectAnchor connectAnchor)
      throws Exception
   {
      ManagedObjectReference vmMor = null;
      int numEthernetCards = -1;
      Vector<ManagedObjectReference> hostVMs = this.iHostSystem.getVMs(
               this.hostMor, null);
      for (ManagedObjectReference mor : hostVMs) {
         numEthernetCards = DVSUtil.getAllVirtualEthernetCardDevices(mor,
                  connectAnchor).size();
         String vmName = this.iVirtualMachine.getVMName(mor);
         log.info(" numEthernetCards for VM :" + vmName + " is  :"
                  + numEthernetCards);
