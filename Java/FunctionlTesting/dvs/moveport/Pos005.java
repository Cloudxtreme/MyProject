...
/**
 * Move a DVPort in a early binding DVPortGroup having scope set to VM1, to late
 * binding DVPortGroup having the scope set to VM2.
 */
public class Pos005 extends MovePortBase
{
   ...
   {
      setTestDescription("Move a DVPort in a early binding DVPortGroup having "
               + "scope set to VM1, to late binding DVPortGroup "
               + "having the scope set to VM2.");
   }

   /**
    * Test setup. 1. Create DVS. 2. Get 2 VM's. 3. Create early binding
    * DVPortGroup with VM-1 in scope and use a DVPort in it as port to be moved.
    * 4. Create late binding DVPortGroup with VM-2 in scope and use it as
    * destination.
    * 
      ...
      DVPortgroupConfigSpec aPortGroupCfg = null;
         ...
         if (super.testSetUp()) {
            ...
            if (hostMor != null) {
               ...
               // vms = ivm.getAllVMs(ihs.getHostFolder(hostMor));
               if ((vms != null) && (vms.size() >= 2)) {
                  vm1Mor = vms.get(0);// get the first VM.
                  vm2Mor = vms.get(1);// get the second VM.
                  log.info("Adding early binding DVPortgroup with "
                           + "VM1 in scope..");
                  ...
                  if (portgroupKeys != null) {
                     ...
                     log.info("Adding late binding DVPortgroup with "
                              + "VM2 in scope..");
                     aPortGroupCfg = buildDVPortgroupCfg(
                              DVPORTGROUP_TYPE_LATE_BINDING, 1, null, vm2Mor);
                     portgroupKeys = addPortgroups(dvsMor, aPortGroupCfg);
                     if ((portgroupKeys != null) && (portgroupKeys.size() > 0)) {
                        log.info("Successfully added late bind port group.");
    ...
    */
   @Override
   @Test(description = "Move a DVPort in a early binding DVPortGroup having "
               + "scope set to VM1, to late binding DVPortGroup "
               + "having the scope set to VM2.")
   public void test()
      throws Exception
