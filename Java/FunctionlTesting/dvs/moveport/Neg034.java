 ...
 * with livePortMovingAllowed = false, to an late binding DVPortgroup with
 * livePortMovingAllowed = true. </br> Procedure:</br> Setup:</br> 1. Create a
 * DVS.</br> 2. Get A VM, used for connecting the DVPort to be moved.</br> 3.
 * Create early binding DVPortgroup with livePortMovingAllowed=false and connect
 * the VM to a DVPort in it and use it as port to be moved.</br> 4. Power on the
 * VM.</br> 5. Create late binding DVPortGroup with livePortMovingAllowed=true
 * and use it as destination.</br> Test:</br> 6. Move the DVPort to destination,
 * Should fail.</br> Cleanup:</br> 7. Delete the DVS and logout.</br>
 */
public class Neg034 extends MovePortBase
{
   ...
   public void setTestDescription()
   {
      setTestDescription("Move a DVPort connected to a powered on VM from "
               ...
               + "livePortMovingAllowed = false, to a late binding "
               + "DVPortgroup with livePortMovingAllowed = true.");
   }
               ...
               if (portgroupKeys != null) {
                  ...
                  if ((portKeys != null) && (portKeys.size() >= 1)) {
                     ...
                     if ((vm1DeltaCfgSpec != null)
                        ...
                        aPortGroupCfg = buildDVPortgroupCfg(
                                 DVPORTGROUP_TYPE_LATE_BINDING, 0, policy, null);
                        portgroupKeys = addPortgroups(dvsMor, aPortGroupCfg);
                        if ((portgroupKeys != null)
      ...
      throws Exception
   {
      ...
      /* Move the DVPort connected to powered on VM */
      try {
