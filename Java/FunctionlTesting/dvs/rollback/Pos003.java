...
public class Pos003 extends TestBase {
	...
	public void setTestDescription() {
		super.setTestDescription("Call Rollback on vDS that is reconfigured with saved config prior to reconfig"
				+ " 1. Create 2 DVS (DVS1 and DVS2)\n"
				+ " 2. export DVS1 and DVS configuration"
				+ " 3. reconfigure DVS1 and DVS2 and change the names\n"
				+ " 4. Call rollback with saved config in step2\n"
				+ " 5. Call reconfigure with the spec returned by rollback operation\n");
	}
	 ...
	 */
	@Override
	@Test(description = "Call Rollback on vDS that is reconfigured with saved config prior to reconfig\n"
			+ " 1. Create 2 DVS (DVS1 and DVS2)\n"
	...
	public void test() throws Exception {
		...
		try {
			...
			if (freePnics != null && freePnics.length >= 2) {
				 ...
				 */
				pgName = conifgInfo2.getName();
				cs = iDvPortgroup.dvPortgroupRollback(dvpg2Mor, dvpg2Config[0]);
				portbefores = conifgInfo1.getNumPorts();
				log.info("ports before reconfig is: " + portbefores);
				cs1 = iDvPortgroup
						.dvPortgroupRollback(dvpg1Mor, dvpg1Config[0]);

				int portsafter = cs1.getNumPorts();
				log.info("ports after reconfig is: " + portsafter);
				log.info("The config name rollback returned is "
						+ cs1.getNumPorts());
				...
				iDvPortgroup.reconfigure(dvpg1Mor, cs1);
				iDvPortgroup.reconfigure(dvpg2Mor, cs);
	...
	public boolean reconfigureDVPortgroup(ManagedObjectReference dvpgMOR)
		...
		DVPortgroupConfigInfo configInfo = iDvPortgroup.getConfigInfo(dvpgMOR);
		boolean status = false;
		if (dvpgMOR != null) {
			if (configInfo.getType().equals(
				...
				dvPortgroupConfigSpec.setName("Pos002-PG");
			}
			dvPortgroupConfigSpec.setConfigVersion(this.iDvPortgroup
					.getConfigInfo(dvpgMOR).getConfigVersion());
		} else {
			log.error("Unable to reconfigure");
		}
		iDvPortgroup.reconfigure(dvpgMOR, dvPortgroupConfigSpec);
		return status;
	}

	public boolean deleteDVSConfiguration(
			Vector<ManagedObjectReference> dvsMORList) throws Exception {
		boolean status = true;
		try {
