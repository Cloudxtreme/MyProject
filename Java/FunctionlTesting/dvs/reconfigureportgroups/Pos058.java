...
/**
 * Reconfigure a portgroup to an existing distributed virtual switch with a max
 * value for the vlanid set in the default port config.
 */
public class Pos058 extends TestBase
{
   ...
   public void setTestDescription()
   {
      setTestDescription("Reconfigure a portgroup to an existing distributed "
               + "virtual switch with a max value for the vlanid set "
      ...
      throws Exception
   {
      log.info("Test Begin:");
      boolean status = false;
      VmwareDistributedVirtualSwitchVlanIdSpec vlanspec = null;
      try {
         vlanspec = new VmwareDistributedVirtualSwitchVlanIdSpec();
         vlanspec.setVlanId(4094);
