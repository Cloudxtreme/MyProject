 ...
 * string. - DVSConfigSpec.name is set to "CreateDVS-Neg017" -
 * DVSConfigSpec.maxPort set to a valid number more than the number of pnics per
 * host. - DVSConfigSpec.numPort set to a valid number less than the number of
 * uplinks ports per host - DVSConfigSpec.uplinkPortPolicy set to a valid array
 * that has number of elements more than the number of pnics being added.
 * ({uplink1,...,uplinkn}) -
...
public class Neg017 extends CreateDVSTestBase
{
   ...
   public void setTestDescription()
   {
      super.setTestDescription("Create DVSwitch with the following parameters"
               ...
               + "- DVSConfigSpec.name is set to 'CreateDVS-Neg017',\n"
         ...
         if (super.testSetUp()) {
            ...
            if (this.networkFolderMor != null) {
               ...
               if (physicalNics != null) {
                  ...
                  uplinkPortNames = new String[physicalNics.length + 4];
                  for (int i = 0; i <= physicalNics.length + 3; i++) {
                     uplinkPortNames[i] = "uplink" + i;
                  }
                  this.configSpec.setMaxPorts(uplinkPortNames.length);
