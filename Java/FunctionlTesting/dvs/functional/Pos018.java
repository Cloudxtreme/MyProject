...
/**
 * For a DRS enabled clustered host with powered on VM's that connects to a
 * standalone DVPort, late binding portgroup, early binding portgroup put the
 * host into maintenance mode.
 */
public class Pos018 extends FunctionalTestBase
{
   ...
   private static final int timeout = TestConstants.EXIT_MAINTMODE_DEFAULT_TIMEOUT_SECS;
                  ...
                  if (this.clusterMor != null) {
                     ...
                     if (setUpDone) {
                        ...
                        if (hostVms != null) {
                           ...
                           if (setUpDone) {
                              log.info("Succesfully powered off all the Vms's"
                                       + " on the host " + this.hostName);
                           } else {
                              ...
                              if ((hostVms != null)
                                 ...
                                 if (setUpDone) {
                                    setUpDone = this.ivm.powerOnVMs(
                                             this.vmMors, checkGuest);
                                    if (setUpDone) {
                                       log.info("Powered on all the VM's on "
                                                + "the host " + this.hostName);
                                    } else {
                                       log.error("Can not power on the "
                                                + "VM's");
                                    }
                                 }
                              } else {
               ...
               if (cleanUpDone) {
                  ...
                  for (ManagedObjectReference vmMor : this.vmMors) {
                     if (this.otherHostName.equals(this.ivm.getHostName(vmMor))) {
                        if (this.ihs.isHostInMaintenanceMode(this.hostMor)) {
                           if (!this.ihs.exitMaintenanceMode(this.hostMor,
                                    timeout)) {
                              cleanUpDone &= false;
                              log.error("Can not exit the hodt from "
                                       + "maintenance mode " + this.hostName);
                           } else {
               ...
               if (this.ihs.enterMaintenanceMode(this.otherHostMor,
                  ...
                  if (!this.icr.moveInto(this.prevOtherHostCluster,
                     ...
                     log.info("Successfully moved the host into the original "
                              + "cluster");
                     if (this.ihs.isHostInMaintenanceMode(this.otherHostMor)) {
                        if (!this.ihs.exitMaintenanceMode(this.otherHostMor,
                                 timeout)) {
                           cleanUpDone &= false;
               ...
               if (this.ihs.isHostInMaintenanceMode(this.hostMor)) {
                  if (!this.icr.moveInto(this.prevHostcluster,
                     ...
                     log.info("Successfully moved the host into the original "
                              + "cluster");
                     if (this.ihs.isHostInMaintenanceMode(this.hostMor)) {
                        if (!this.ihs.exitMaintenanceMode(this.hostMor, timeout)) {
                           cleanUpDone &= false;
                           log.error("Can not move the host out of the "
                                    + "maintenance mode " + this.hostName);
                        } else {
