 ...
 * connected to a standalone port on the DVS and the port is not occupied, while
 * reverting the snapshot.
 */
public class Pos001 extends VMSFunctionalTestBase
{
   /**
   ...
   public void setTestDescription()
   {
      setTestDescription("Revert a powered off VM to a snapshot when it's VM "
               + "ethernet adapter was connected to a standalone port"
               + " on the DVS and the port is not occupied, while"
               + " reverting the snapshot.");
   }
