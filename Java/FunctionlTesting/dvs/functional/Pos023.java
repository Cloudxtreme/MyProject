...
/**
 * Clone a VM from a powered off VM that is connected to the standalone DV Port
 * of the DVSwitch. The VM can be powered on and the VM will not have network
 * connectivity.
 */
public class Pos023 extends FunctionalTestBase
{
   ...
   {
      setTestDescription("Clone a VM from a powered off VM that is connected"
               + " to the standalone DV Port of the DVSwitch. The "
               + "VM can be powered on and the VM will not have "
    ...
    * @param connectAnchor ConnectAnchor object
    */
   @Test(description = "Clone a VM from a powered off VM that is connected"
               + " to the standalone DV Port of the DVSwitch. The "
               + "VM can be powered on and the VM will not have "
               + "network connectivity.")
   public void test()
      throws Exception
   {
      ...
      String ipAddress = null;
         ...
         this.cloneVMMor = this.ivm.cloneVM(this.vmMor, this.ivm.getVMFolder(),
                  this.getTestId() + "-Clone1", cloneSpec);
               ...
               if (testDone) {
                  ...
                  if (checkGuest) {
                     ...
                     if (ipAddress != null) {
                        ...
                        log.info("The VM doesnot have network connectivity");
                     }
                  }
               } else {
                  log.error("Can not power on the VM "
