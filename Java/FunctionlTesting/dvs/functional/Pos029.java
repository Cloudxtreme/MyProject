...
/**
 * Clone a VM from a powered on VM(VM1) that is connected to a port on an early
 * binding port group, that has twice the number of ports than those used by the
 * VM(VM1) connected to the DVSwitch. The VM can be powered on. The VM will have
 * network connectivity.
 */
public class Pos029 extends FunctionalTestBase
{
   ...
   {
      setTestDescription("Clone a VM from a powered on VM(VM1) that is "
               + "connected to a port on an early binding port group, "
               + "that has twice the number of ports than those used "
               + "by the VM(VM1) connected to the DVSwitch. The VM "
               + "can be powered on. The VM will have network "
               + "connectivity.");
   }
    ...
    * @param connectAnchor ConnectAnchor object
    */
   @Test(description = "Clone a VM from a powered on VM(VM1) that is "
               + "connected to a port on an early binding port group, "
               + "that has twice the number of ports than those used "
               + "by the VM(VM1) connected to the DVSwitch. The VM "
               + "can be powered on. The VM will have network "
      ...
      throws Exception
   {
      ...
      String ipAddress = null;
         ...
         assertTrue(InternalDVSHelper.verifyPortPersistenceLocation(hostConnectAnchor,
                  this.ivm.getName(this.vmMor), this.dvSwitchUUID),
                  "Verification for PortPersistenceLocation failed");
         DVSUtil.reconfigureWithTrafficShapingPolicy(connectAnchor, this.dvsMor);
         hostConnectAnchor = new ConnectAnchor(this.ihs.getHostName(hostMor),
                  data.getInt(TestConstants.TESTINPUT_PORT));
         assertTrue(InternalDVSHelper.verifyPortPersistenceLocation(hostConnectAnchor,
                  this.ivm.getName(this.vmMor), this.dvSwitchUUID),
                  "Verification for PortPersistenceLocation failed");
         ...
         /*
          * TODO cleanup this code to use the best data store on the host
          */
