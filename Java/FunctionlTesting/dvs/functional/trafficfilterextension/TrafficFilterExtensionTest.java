package dvs.functional.trafficfilterextension;
...
public class TrafficFilterExtensionTest extends TestBase implements IDataDrivenTest
{

   private Object TrafficFilterExtensionTestFramework = null;
    ...
    */
   @Override
   @Test(description="Set of all tests for TrafficFilterExtension api")
   public void test()
      throws Exception
   {
      ...
      try {
         Method method =
               testFrameworkClass.getDeclaredMethod("execute", List.class);
         method.invoke(this.TrafficFilterExtensionTestFramework, stepList);
            ...
            if (ex2 instanceof InvocationTargetException) {
               try {
                  ...
                  assertTrue(TestUtil.checkMethodFault(actualMethodFault,
                        ...
                        + "faults match",
                        "The expected and actual method faults "
                              + "do not match");
               }
            }
         }
      ...
      throws Exception
   {
      if (DVSUtil.getvDsVersion().compareTo(DVSTestConstants.VDS_VERSION_55) < 0) {
          log.error("cannot execute traffic filter extension test " +
              "cases prior to dvs version 5.5.0");
          throw new Exception("cannot execute traffic filter extension test " +
          "cases prior to dvs version 5.5.0");
      }
      xmlTester = new StepReader(this.data);
      String testFrameworkClassName =
            xmlTester.getData(DVSTestConstants.TEST_FRAMEWORK);
      String beanPath = xmlTester.getData(DVSTestConstants.BEAN_DATA);
      testFrameworkClass = Class.forName(testFrameworkClassName);
      Constructor<?> constructor =
            testFrameworkClass
                  .getConstructor(ConnectAnchor.class, String.class);
      TrafficFilterExtensionTestFramework =
            constructor.newInstance(this.connectAnchor, beanPath);
   }
