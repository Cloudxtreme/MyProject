...
/**
 * Create a vds version 5.1, add a host with free pnic to the vds and
 * reconfigure the uplink portgroup to enable/disable lacp
 */
public class Pos002 extends CreateDVSTestBase implements IDataDrivenTest
{
   ...
   @Test(description = "Create a vds, add a host with free pnic "
            + "to the vds and reconfigure the uplink portgroup to " +
            "enable / disable lacp.")
   public void test()
      throws Exception
   {
      if (this.configSpec != null) {
         ...
         if (this.dvsMOR != null) {
            ...
            dvpgConfigSpec.setConfigVersion(dvpgConfigInfo.getConfigVersion());
            VMwareDVSPortSetting setting = new VMwareDVSPortSetting();
            setting.setLacpPolicy(lacpPolicy);
            dvpgConfigSpec.setDefaultPortConfig(setting);
            this.idvpg.reconfigure(pgMorList.get(0), dvpgConfigSpec);
            log.info("Successfully reconfigured the uplink portgroup with " +
                  "lacp policy");
            dvpgConfigInfo = this.idvpg.getConfigInfo(pgMorList.get(0));
