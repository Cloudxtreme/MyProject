 ...
 * to an ephemeral portgroup of the DVSwitch to a different datastore on a host
 * that has multiple datastores. The VM moves to the destination datastore.
 */
public class Pos007 extends SVMFunctionalTestBase
{

   public void setTestDescription()
   {
      setTestDescription("Storage vmotion a powered on VM via a vmotion virtual"
               + " nic that is connected to a ephemeral  portgroup of "
         ...
         if (super.testSetUp()) {
            ...
            if (this.vmotionSystem != null) {
               ...
               portgroupKey = this.iDVS.addPortGroup(this.dvsMor,
                        DVPORTGROUP_TYPE_EPHEMERAL, 2, this.getTestId()
                                 + "-lpg");
