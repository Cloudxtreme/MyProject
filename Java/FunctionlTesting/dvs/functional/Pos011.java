 ...
 * DVSwitch), after a timeout to be determined, assign an DVPort on an early
 * binding DVPortGroup that was previously used by a VM on the host to be used
 * by another VM on another host that was previously added to the DVSwitch.
 * Power on the VM.
 */
public class Pos011 extends FunctionalTestBase
{
   ...
   {
      setTestDescription("After a host leave (the host got disconnected/ the"
               + " host was removed from the DVSwitch), after a "
               + "timeout to be determined, assign an DVPort on an"
               + " early binding DVPortGroup that was previously used"
               + " by a VM on the host to be used by another VM on "
               + "another host that was previously added to the "
               + "DVSwitch. Power on the VM.");
   }
    ...
    * @param connectAnchor ConnectAnchor object
    */
   @Test(description = "After a host leave (the host got disconnected/ the"
               + " host was removed from the DVSwitch), after a "
               + "timeout to be determined, assign an DVPort on an"
               + " early binding DVPortGroup that was previously used"
               + " by a VM on the host to be used by another VM on "
         ...
         if (vmConfigSpec != null && vmConfigSpec.length == 2
            ...
            if (testDone) {
               ...
               if (testDone) {
                  ...
                  testDone =
                           this.ivm.setVMState(this.otherVMMor, VirtualMachinePowerState.POWERED_ON, false);
