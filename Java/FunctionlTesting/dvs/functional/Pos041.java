...
/**
 * PR# 395300 : test case 1 1. Create a DVS with 1 static, 1 dynamic, 1
 * ephemeral PGs (say 2 ports each) 2. Add 2 hosts to it 3. Create a VM on host1
 * 4. Create 3 virtual-adapters on VM 5. Reconfigure VM to move vm-vmic1 to
 * static, vm-vnic2 to dynamic and vm-vnic3 to ephemeral PG 6. Power on VM,
 * check VMs have networking, if Yes, power off VM, if No, flag error 7.
 * Reconfigure VM (pass the SAME spec as current config, i.e. vm-vnic1 to
 * static, vm-vnic2 to dynamic and vm-vnic3 to ephemeral PG). 8. Reconfigure all
 * 3 Portgroups 9. Power on VM, check VMs have networking, if Yes, power off VM,
 * if No, flag error
 */
public class Pos041 extends TestBase
{
   ...
   {
      super.setTestDescription("PR# 395300  : test case 1 \n"
    ...
    */
   @Override
   @Test(description = "PR# 395300  : test case 1 \n"
               + "1. Create a DVS with 1 static, 1 dynamic, 1 ephemeral PGs (say 2 ports each) \n"
               + " 2. Add 2 hosts to it \n"
               + " 3. Create a VM on host1 \n"
               + " 4. Create 3 virtual-adapters on VM \n"
      ...
      throws Exception
   {
      ...
      if (poweredOn) {
         ...
         /*
          * Verifying  port linkup state change without the RefreshPortState call.
          * Testcase should wait 11 minutes for Pre-MN host and around 1 minute for MN host(Bug#661062)
          */

         if (TestUtil.compareVersion(this.ihs.getHostVersion( this.ivm.getHost(vmMor)),
                  VersionConstants.MN_BASE_VERSION) >= 0) {
            sleepTime = DVSTestConstants.PORT_REFRESH_TIME_MNHOSTS;
         } else {
            sleepTime = DVSTestConstants.PORT_REFRESH_TIME_PRE_MNHOSTS;
         }
      ...
      throws Exception
   {
      ...
      if (pgList != null && pgList.size() == 1) {
         ...
         log.info("Successfully added the early binding "
                  + "portgroup to the DVS " + this.getTestId() + "-epg");
         pgMor = pgList.get(0);
